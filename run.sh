...
if ! command -v "$PYTHON_BIN" >/dev/null 2>&1 \
    ...
    log "Bootstrapping Python (apt-get install python3 python3-venv python3-pip ...)"
...
print(f"  device picked   : {'cuda' if torch.cuda.is_available() else 'cpu'}")
PY
...
if [ -f "node_embeddings.pt" ] && [ "${FORCE_RETRAIN:-0}" != "1" ]; then
    log "node_embeddings.pt already present (committed) — skipping GNN training."
    log "    set FORCE_RETRAIN=1 to retrain from scratch."
else
    log "Training GraphSAGE+GAE encoder (this can take several minutes on CPU)..."
...
if [ "${SKIP_INDIVIDUAL:-0}" = "1" ]; then
    ...
    # run_agent <human-name> -- <args to train_rl.py>
    run_agent() {
        local name="$1"; shift
        local agent_log="$LOG_DIR/agent_${name}.log"
        log "  → ${name} :: train_rl.py $*"
        local t0; t0=$(date +%s)
        # Keep going on agent-level failures so one bad agent doesn't kill the run.
        if python train_rl.py --device "$DEVICE" --seed "$SEED" "$@" \
                > "$agent_log" 2>&1; then
            local dt=$(( $(date +%s) - t0 ))
    ...
    # ── Baselines & centrality (sub-second to ~1 min)
    ...
    # ── Reverse Influence Sampling (theta capped for runtime safety)
    run_agent imm   --agent imm   --budget "$BUDGET" --mc-sims "$MC_SIMS" --imm-theta-max 50000

    # ── Simulation-based greedy (CELF is much faster than naive greedy_mc)
    ...
    # ── Learning agents (reduced episodes vs the original benchmark for time)
    ...
    # ── Learning DL (Structure2Vec + DQN, jointly trained)
...
if [ "${SKIP_BENCHMARK:-0}" = "1" ]; then
    ...
    log "Running scripts/compare_all.py over all agent families."
    log "    (excluding celfpp, louvain, betweenness — these take 1-4 hours on CPU)"
...
Artifacts produced (all paths relative to project root):
  • Embeddings ............ node_embeddings.pt   node_embeddings.npy
  • GNN metadata .......... embeddings_meta.json
  • GNN checkpoint ........ checkpoints/best_model.pt
  • Per-agent results ..... ${RESULTS_DIR}/<timestamp>_<agent>.json   (one per agent)
  • Benchmark JSON ........ ${LATEST_BENCHMARK:-(none — STAGE 6 was skipped)}
  • Markdown run report ... docs/RUN_REPORT.md
  • Reference report ...... docs/COMPREHENSIVE_BENCHMARK.md  (curated, full 14-agent run)
  • Logs .................. ${LOG_DIR}/

Quick peek at the latest benchmark headline (top 6 by mean spread):
EOF
